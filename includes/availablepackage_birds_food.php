...
//
			...
			if(empty($name) || empty($prize) || empty($des) || empty($stat)){
					echo '<script>alert("Fields must be empty.");
								 window.location.href="addcnp2_food.php";
					</script>';
				
				}else {
					$sql = ("INSERT INTO tblcnp_birds_food VALUES(NULL,'$name','$prize','$des','$location','$stat')") or die (mysql_error());
                    $result=mysqli_query($con, $sql);
					if($result==true){
						echo '<script>alert("Save Successfully!");
									window.location.href="addcnp2_food.php";</script>';}else {
										echo '<script>alert("Sory unable to process your request!");
									window.location.href="addcnp2_food.php";</script>';
										}
