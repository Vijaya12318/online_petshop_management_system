...
<!--*********************************************START OF NAVIGATION BAR****************************************--> 
        ...
        $count = 0;
        $id = 0;
        $sql = "SELECT * FROM tblcnp_dogs WHERE status = 'available' order by id desc" or die (mysqli_error($con));
...
if(isset($_POST['savechanges'])){
		...
		$sql =("SELECT * FROM tblorders_dogs WHERE cname = '$name'") or die (mysqli_error());
    $result=mysqli_query($con, $sql);
			if(mysqli_num_rows($result)==5){
				...
				$sql = ("INSERT INTO tblorders_dogs VALUES(NULL,'$name','$address','$contact','$id','$qty','new',NULL,'$otype','$datep')") or die (mysqli_error());
        $result=mysqli_query($con, $sql);
							if($result==true){
								echo '<script>alert("Your order will be process.The system will follow up by contacting your number thankyou!");
											 window.location.href="availableframe_dogs.php"</script>';}
											 else{
