...
<div class="container">
   <form id="formFilter" name="formFilter" action="admin_reservefilter_cats.php" method="POST" class="pull-left col-md-7 hidden-print">
        ...
        <table class="table table-hover table-condensed wow fadeInDown">
            ...
            <tbody id="tablebody">
			   ...
			   $sql = ("SELECT tblcnp_cats.*,tblorders_cats.* 
											...
											tblcnp_cats ON tblorders_cats.cnpoid = tblcnp_cats.id WHERE tblorders_cats.ostatus = 'New' order by tblorders_cats.id LIMIT 0,30 ") or die (mysqli_error());
...
<div class="modal fade" id="viewModal<?php echo $row['id']?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
 ...
 <!-------------------------------------------------------OPEN MODAL MESSAGE---------------------------------------------------------------->
	   ...
	   if(isset($_POST['deliver'])){
		   $id = $_POST['fdid'];
		   $sql =("UPDATE tblorders_cats set ostatus = 'Completed' WHERE id = '$id'") or die (mysqli_error());
       ...
       if($result==true){
			 ...
			 $id = $_POST['fdid'];
			 $sql = ("UPDATE tblorders_cats set ostatus = 'Cancel' WHERE id = '$id'") or die (mysqli_error());
			 $result=mysqli_query($con, $sql);
       if($result== true){
				 header("location:index.php");}
			 }
